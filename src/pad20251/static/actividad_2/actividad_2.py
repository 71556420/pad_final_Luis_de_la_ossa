...
output_dir = "src/pad20251/actividad_2"
...
# Genera los valores de x y y para el gráfico de contorno
...
# Aplana Z para asegurar que tenga la misma longitud que X y Y
X_flat = X.flatten()
Y_flat = Y.flatten()
Z_flat = Z.flatten()

# Verifica las longitudes de las matrices
print(len(X_flat), len(Y_flat), len(Z_flat))  # Esto debería imprimir 10000

# Crea el DataFrame con las coordenadas y los valores calculados
df_contorno = pd.DataFrame({'x': X_flat, 'y': Y_flat, 'z': Z_flat})

# Guardar los resultados en un archivo Excel
with pd.ExcelWriter('resultados_actividad_6_a_10.xlsx', engine='openpyxl') as writer:
    # Guarda la hoja de contorno Z
    df_contorno.to_excel(writer, sheet_name='Contorno Z', index=False)
